...
(function () {
  ...
  function renderGroupGrades(entry, payload) {
    var subjectsHtml = (payload.subjects || []).map(function (subj) {
      ...
      var srsHead = srsEnabled ? subj.srsTitles.map(function (t) { return '<th>' + escapeHtml(t) + '</th>'; }).join('') : '';
      var rows = subj.students.map(function (s) {
        ...
        var srsCells = srsEnabled ? s.srsScores.map(function (v) { return '<td>' + scoreCell(v) + '</td>'; }).join('') : '';
        return '<tr><td>' + escapeHtml(s.name) + '</td>' +
          ...
          (srsEnabled ? '<td>' + scoreCell(s.srsAvg) + '</td>' : '') +
      ...
      return '<div class="subject-card">' +
        ...
        '<th>Лекции ср.</th><th>Задания ср.</th>' + (srsEnabled ? '<th>СРС ср.</th>' : '') + '<th>Итог</th></tr></thead>' +
